/*
  embed.js - used to embed templates into the document
*/

/*
  Module dependencies
*/
var fs = require('fs'),
    path = require('path'),
    join = path.join,
    extname = path.extname,
    basename = path.basename,
    normalize = path.normalize,
    cheerio = require('cheerio'),
    thimble = require('../thimble'),
    utils = thimble.utils,
    read = utils.read,
    after = utils.after,
    escapeJSON = utils.escapeJSON,
    support = normalize(__dirname + '/../../support');
    
/*
  Template cache
*/

var cache = {};

/*
  Requires caches
*/

var requires = {};
    
/*
  Export module
*/

exports = module.exports = function(opts) {
  opts = opts || {};

  return function(content, options, next) {
    var $ = cheerio.load(content),
        $scripts = $('script'),
        $javascripts = $('script[type=text/javascript]'),
        length = $scripts.length - $javascripts.length,
        finished;
    
    if(length) {
      finished = after(length);
    } else {
      return next(null, content);
    }
    
    $scripts.each(function() {
      if($(this).attr('type') === 'text/javascript') return;
      
      var $script = $(this),
          source = $script.attr('src'),
          ext = extname(source),
          id = $script.attr('id'),
          name = (id) ? id : basename(source, ext),
          assetPath = join(options.root, thimble.path.convert(source, options)),
          namespace = opts.namespace || options.namespace || 'window',
          key = options.template || 'JST',
          precompile;

      // Map to known compiles, example: "mu" => "hogan"
      ext = ext.substring(1);
      ext = (thimble.extensions[ext]) ? thimble.extensions[ext] : ext;
      
      // Allow custom mappings for namespace keys window.json = ...
      key = (opts[ext]) ? opts[ext] : key;
      
      precompile = exports[ext];
      
      // If there's no precompiler, either move on or return if finished
      if(!precompile) {
        if(finished()) return next(null, $.html());
        return;
      }

      precompile(assetPath, options, function(err, str) {
        if(err) return next(err);
        
        $script.removeAttr('src')
               .attr('type', 'text/javascript');

        // Wrap the string in boilerplate
        str = wrap(namespace, key, name, str);
        
        $script.text(str);
        
        if(finished()) return next(null, $.html());
      });
      
    });
    
  };
};

/*
  Wrap our embeds
*/
var wrap = function(namespace, type, name, content) {
  var out = [],
      exports = namespace + '.' + type;
  
  // namespace.type =
  out.push('\n' + exports + ' = ');
  
  // namespace.type || {}
  out.push(exports + ' || {}; ');
  
  // namespace.type['name'] = 
  out.push(exports + '[\'' + name + '\'] = ');
  
  // (function() {
  out.push('(function() {\n');
  
  // content
  out.push(content);
  
  // })();
  out.push('})();');
  
  return out.join('');
};

/*
  Precompilers
*/

/*
  Hogan.js - Fast mustache templating
*/
exports.hogan = function(file, options, fn) {
  var engine = requires.hogan || (requires.hogan = require('hogan.js')),
      out = [];
  
  // Add hogan library as support file
  var dep = {
    file : join(support, 'hogan.js'),
    appendTo : 'head'
  };
  options.support.push(dep);
  
  read(file, function(err, str) {
    if(err) return fn(err);

    // Need to bind, because render loses object
    // Call template on clientside with: JST['template'](locals)
    out.push('  var __bind = function(fn, me) {');
    out.push('    return function() { return fn.apply(me, arguments); };');
    out.push('  },');
    out.push('  tpl = new Hogan.Template(');
    
    out.push('    ' + engine.compile(str, {asString : true}));
    
    out.push('  );');
    out.push('  return __bind(tpl.render, tpl);');
        
    return fn(null, out.join('\n'));
    
  });
};

/*
  JSON - embed JSON into our document
*/
exports.json = function(file, options, fn) {
  read(file, function(err, str) {
    // Normalize
    try {
      var obj = JSON.parse(str);
      str = JSON.stringify(obj);
      str = escapeJSON(str);
    } catch(err) {
      return fn(err);
    }
    str = "return JSON.parse(" + str + ");";

    return fn(null, str);
  });
};
    