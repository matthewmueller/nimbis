...
var start = exports.start = function(app) {
  if(!app || !app.use) 
    throw new Error('Instance of express required for thimble.start');
    
  var server = require("./server");

  // Pull in settings from express
  if(!this.settings.root && app.settings.views)
    this.settings.root = app.settings.views;
  
  if(!this.settings['view engine'] && app.settings['view engine'])
    this.settings['view engine'] = app.settings['view engine'];
  else
...
var eval = exports.eval = function(content, locals, fn) {
  ...
  if (options.source && options.compile) {
    self.stack.push(self.compile(options.source, options.locals));
  }
